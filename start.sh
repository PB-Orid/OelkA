...
## remove pidfiles if termination failed
rm -f /var/run/elasticsearch/elasticsearch.pid /var/run/logstash.pid \
  /var/run/kibana5.pid

## initialise log files to stream in console
OUTPUT_LOGFILES=""
...
if [ -x /usr/local/bin/elk-post-hooks.sh ]; then
  ...
  # wait... 4 Kibana to start first and see .kibana index is
  # created before the of post-hooks exe.
    # retries (default: 30, override using KIBANA_CONNECT_RETRY env var)
